...
contract TreasuryVaultAave is Ownable, Initializable, ReentrancyGuard {
    ...
    event IncentiveControllerUpdated(address indexed newController);
    ...
    function deposit(uint256 _amount) external onlyTreasury nonReentrant {
    ...
    function setIncentiveController(address _aaveIncentivesController) external onlyOwner {
        ...
        AaveIncentivesController = IAaveIncentivesController(_aaveIncentivesController);
        emit IncentiveControllerUpdated(_aaveIncentivesController);
    }
